...
class HomeRepository implements HomeRepositoryInterface
{
    ...
    public function shopDetails($input,$user,$lang)
    {
        $shop = Shop_detail::where("shop_id",$input->shop_id)
            ->with(["phones" => function($query) use($lang){
                ...
                $query->select('id','name_'.$lang." as name","image","old_price","new_price",
    ...
    {
        return Offer::where("id",$input->offer_id)
    ...
    public function addReview($user_id,$input,$lang)
    {
        ...
        if(!($blockCheck)){
            ...
            $reviewer=User::where("id",$id)->first()->name;
            if($lang == "en"){
                $title="E3LN";
    ...
    {
        $add                    =new Offer();
        $add->name_en           =$input->name;
        $add->name_ar           =$input->name;
        $add->image             =$input->image;
        $add->old_price         =$input->old_price;
        $add->new_price         =$input->new_price;
        $add->description_en    =$input->description;
        $add->description_ar    =$input->description;
        $add->shop_id           =$shop_id;
        $add->save();

        $addedOffer = Offer::where("shop_id","$shop_id")
            ->orderBy('id','desc')
            ->select('id','name_'.$lang.' as name','image','old_price','new_price','description_'.$lang.' as description')->first();
            ...
            foreach($shops as $shop){
                $offer=Offer::where("shop_id",$shop->shop_id)
                    ...
                    ->select("id","name_".$lang." as name","image","old_price","new_price",
                        "description_".$lang." as description","description_".$lang." as description")
                    ->get();
                if(sizeof($offer) > 0){
                    foreach ($offer as $o){
                        array_push($offers,$o);
