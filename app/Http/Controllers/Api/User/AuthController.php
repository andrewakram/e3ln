...
class AuthController extends Controller
{
        ...
        if( $data=checkJWT($request->header('jwt')) ){
            if($data->is_shop == 0){
                $user = $this->authRepository
                    ->beAShop($request,$data->email,$request->header('jwt'),$request->header('lang'));
                if($user)
                {
    ...
    public function codeSend(Request $request)
    {
        ...
        $this->authRepository->sendSMS($request->type,$request->phone);
        ...
        if($check)
        {
            if(Carbon::now()->format('Y-m-d H') > Carbon::parse($check->expire_at)->format('Y-m-d H'))
            {
                ...
                if($check->type == 'activate')
                {
                    ...
                    $user = $this->authRepository->userData($user2->jwt,0,$request->header('lang'));
    ...
    public function login(Request $request)
    {
        ...
        if($user)
        {
            if(Hash::check($request->password,$user->password))
            {
                ...
                $user->save();
                $user = $this->authRepository->userData($user->jwt,$user->is_shop,$request->header('lang'));
